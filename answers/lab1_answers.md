...
- Sự cố C -> Availability (Tính khả dụng)

---

## 3. Phân tích sự cố B
- Threat: Tin tặc (Hacker) hoặc nhân viên nội bộ có ý đồ xấu muốn thay đổi thông tin để trục lợi.
- Vulnerability:Hệ thống thiếu kiểm soát truy cập (Access Control) chặt chẽ, không có cơ chế kiểm tra tính toàn vẹn (Hashing), hoặc tồn tại lỗ hổng SQL Injection.
- Mitigation:Sử dụng các thuật toán băm (SHA-256) để kiểm tra tính toàn vẹn, thiết lập quyền truy cập tối thiểu (Least Privilege) và thực hiện sao lưu (Backup) định kỳ để khôi phục khi cần.
